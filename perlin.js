...
(function(global){
  ...
  var grad3 = [new Grad(1,1,0),new Grad(-1,1,0),new Grad(1,-1,0),new 
Grad(-1,-1,0),
               new Grad(1,0,1),new Grad(-1,0,1),new Grad(1,0,-1),new 
Grad(-1,0,-1),
               new Grad(0,1,1),new Grad(0,-1,1),new Grad(0,1,-1),new 
Grad(0,-1,-1)];
...
length
  ...
  // This isn't a very good seeding function, but it works ok. It 
supports 2^16
  // different seed values. Write something better if you need more 
seeds.
...
unskewed.
    ...
    var i1, j1; // Offsets for second (middle) corner of simplex in 
(i,j) coords
...
coords
    var i2, j2, k2; // Offsets for third corner of simplex in (i,j,k) 
coords
    if(x0 >= y0) {
